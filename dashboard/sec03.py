...
from .kpis        import fmt_manager, fmt_signed_short, render_kpi_small
...
def _semaforo_badge(valor, mapa_colores, valor_num=None, sufijo_num=""):
    """Genera HTML de badge semaforo + valor numérico opcional."""
    color = mapa_colores.get(str(valor).strip(), "neutral") if valor and str(valor).strip() not in ("nan", "None", "") else "neutral"
    label = str(valor).strip() if valor and str(valor).strip() not in ("nan", "None", "") else "—"
    html = f'<span class="badge {color}">{label}</span>'
    if valor_num is not None and pd.notna(valor_num):
        html += f'<br><small style="color:#888;font-size:.65rem">{valor_num:.1f} {sufijo_num}</small>'
    return html


def render_tarjetas_lote(info, nombre_g, lote_codigo, galpon_v, zona_v, tipo_v, repro_v, quint_v, semaforo=None):
...
<div class="sel-pill-neutral">📍 {zona_v} · {tipo_v} · {repro_v} · {quint_v}</div>
    ...
    with c9:
        # ── SEMÁFORO: Reproductora ──────────────────────────────
        rv   = sf.get("reproductora_val")
        edad_r = sf.get("edad_repro_val")
        badge_repro = _semaforo_badge(
            rv,
            {"Joven": "green", "Adulta": "amber", "Vieja": "red"},
            valor_num=edad_r, sufijo_num="sem"
        )
        render_kpi_small(badge_repro, "Reproductora")
    with c10:
        # ── SEMÁFORO: Guarda ────────────────────────────────────
        gv     = sf.get("guarda_val")
        dias_g = sf.get("dias_guarda_val")
        badge_guarda = _semaforo_badge(
            gv,
            {"Optima": "green", "Moderada": "amber", "Critica": "red"},
            valor_num=dias_g, sufijo_num="días"
        )
        render_kpi_small(badge_guarda, "Guarda")
    with c11:
        render_kpi_small(fmt_num(info["gap_costo_kg"], 4, prefix="$"), "Gap $/kg")
    with c12:
        # ── SEMÁFORO: Ideal comparado ───────────────────────────
        etiq   = sf.get("etiqueta_ideal_usada")
        exacto = sf.get("es_match_exacto", False)
        if etiq and str(etiq).strip() not in ("nan", "None", ""):
            color_i = "green" if exacto else "amber"
            sufijo_i = "" if exacto else " ≈"
            badge_ideal = (
                f'<span class="badge {color_i}" style="font-size:.7rem;'
                f'white-space:normal;word-break:break-all">{etiq}{sufijo_i}</span>'
            )
        else:
            badge_ideal = '<span class="badge red">Sin ideal</span>'
        render_kpi_small(badge_ideal, "Ideal comparado")
...
<div>
    ...
    tipo_v   = il["TipoStd"]
    quint_v  = il["Quintil"]  if "Quintil"        in il.index else "—"
    ...
    # ── Datos semáforo ────────────────────────────────────────
    rv_raw   = il.get("ReproductoraRaw") if "ReproductoraRaw" in il.index else None
    if rv_raw is None or str(rv_raw).strip() in ("nan", "None", ""):
        # fallback: usar ReproductoraStd en title case
        rv_raw = repro_v.title() if repro_v and repro_v != "SIN_DATO" else None

    edad_repro  = il.get("ponderado_edad_reproductora") if "ponderado_edad_reproductora" in il.index else None
    guarda_raw  = il.get("Guarda")  if "Guarda"  in il.index else None
    dias_guarda = il.get("ponderado_dias_guarda") if "ponderado_dias_guarda" in il.index else None

    # ── Determinar qué ideal se usó (exacto o aproximado) ────
    etiqueta_lote = il.get("Etiqueta_Escenario") if "Etiqueta_Escenario" in il.index else None
    etiqueta_ideal_usada = None
    es_match_exacto = False

    if IDEALES is not None and not IDEALES.empty and "Etiqueta_Escenario" in IDEALES.columns:
        if etiqueta_lote and str(etiqueta_lote).strip() not in ("nan", "None", ""):
            match_ex = IDEALES[IDEALES["Etiqueta_Escenario"] == str(etiqueta_lote).strip()]
            if not match_ex.empty:
                etiqueta_ideal_usada = str(etiqueta_lote).strip()
                es_match_exacto = True

        if not es_match_exacto:
            # Buscar el más cercano con Zona + Tipo + Quintil
            for filtro in [
                (IDEALES["Zona_Nombre"] == zona_v) & (IDEALES["TipoGranja"] == tipo_v) & (IDEALES["Quintil"] == quint_v),
                (IDEALES["Zona_Nombre"] == zona_v) & (IDEALES["TipoGranja"] == tipo_v),
            ]:
                sub = IDEALES[filtro]
                if not sub.empty:
                    etiqueta_ideal_usada = str(sub["Etiqueta_Escenario"].iloc[0])
                    break

    semaforo = {
        "reproductora_val":    rv_raw,
        "edad_repro_val":      pd.to_numeric(edad_repro, errors="coerce") if edad_repro is not None else np.nan,
        "guarda_val":          guarda_raw,
        "dias_guarda_val":     pd.to_numeric(dias_guarda, errors="coerce") if dias_guarda is not None else np.nan,
        "etiqueta_ideal_usada": etiqueta_ideal_usada,
        "es_match_exacto":     es_match_exacto,
    }
