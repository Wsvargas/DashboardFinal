...
def enriquecer_historial_con_ideal(
    ...
    for lote, hist_lote in base.groupby("LoteCompleto", sort=False):
        ...
        # ── Matching por Etiqueta_Escenario (preferido) ───────────────
        ideal_sub = pd.DataFrame()
        etiqueta = None
        if "Etiqueta_Escenario" in hist_lote.columns:
            etiqueta_series = hist_lote["Etiqueta_Escenario"].dropna()
            if not etiqueta_series.empty:
                etiqueta = str(etiqueta_series.iloc[0]).strip()

        if etiqueta and "Etiqueta_Escenario" in ideales_df.columns:
            ideal_sub = ideales_df[ideales_df["Etiqueta_Escenario"] == etiqueta].copy()

        # ── Fallback al match por 4 claves si no hay etiqueta o no matchea ─
        if ideal_sub.empty:
